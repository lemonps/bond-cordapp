...
//        val iouToSettle = serviceHub.vaultService.queryBy<UserState>(queryCriteria).states.single()
//        val counterparty = iouToSettle.state.data.owner
//
//        // Step 2. Check the party running this flow is the borrower.
////        if (ourIdentity != iouToSettle.state.data.owner) {
...
//        } else if (amount > (iouToSettle.state.data.total - iouToSettle.state.data.paid)) {
//            throw IllegalArgumentException("Borrower tried to settle with $amount but only needs ${ (iouToSettle.state.data.total - iouToSettle.state.data.paid) }")
//        }
//
...
//        val settleCommand = Command(BondContract.Commands.SettleBond(), listOf(counterparty.owningKey, ourIdentity.owningKey))
...
//        val amountRemaining = iouToSettle.state.data.total - iouToSettle.state.data.paid - amount
//        if (amountRemaining > Amount(0, amount.token)) {
//            val settledIOU: UserState = iouToSettle.state.data.pay(amount)
//            builder.addOutputState(settledIOU, BondContract.BOND_CONTRACT_ID)
//        }
//
